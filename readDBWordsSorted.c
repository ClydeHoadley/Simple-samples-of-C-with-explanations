#define Version "{readDBWordsSorted (v1.0a)}"
/****************************************************************************************
readDBWordsSorted.c                                                        12 August 2021
    ...
    readDBWordsSorted.c is a companion program of loadWordTable.c and readDBWords.c
    readDBWordsSorted and readDBWords are identical except for the format of the SELECT
    statement.

    gcc readDBWordsSorted.c `mysql_config –cflags` `mysql_config –libs` -g -Wall \\
        -I/usr/include/mysql  -L/usr/local/mysql/lib -lmysqlclient -o readDBWordsSorted
...
bool logonToDatabase(MYSQL **connect) {
    ...
    *connect=NULL;
    do {
        ...
        *connect= mysql_init(NULL); //initialize a structure for connect to use
        if (connect == NULL) {
...
bool fetchWords(MYSQL *connect) {
    ...
    if ( !connect ) return false;
                           ...
                           //Different from readDBWords.c in that I am requesting
                           //specific columns (fields) and I'm telling the database to
                           //give them to me in ascending order sorted on word (a -> z.)
    if ( mysql_query(connect, "SELECT id, word FROM words order by word asc" ) ) {
        errorExit("Failed to select from table",NULL,connect,NULL);
    }

    result = mysql_store_result(connect); //returns the entire result of the query.
    ...
    while ((row = mysql_fetch_row(result))){  //get 1 row at a time from result set
        rowCount++;
        printf("%5s\t%s\n",row[0],row[1]);
    }
